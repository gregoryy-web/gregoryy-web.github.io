    ...
    if (pageInfo && pageInfo.totalPages > 1) {
        navHtml = `
            <div class="d-flex justify-content-between align-items-center py-3 px-1">
                <button class="btn btn-outline-teal px-3 px-sm-4 rounded-pill prevBtnInst" ${pageInfo.currentPage === 0 ? 'disabled' : ''} style="border-color: #20c997; color: #20c997;">Prev</button>
                <span class="fw-bold text-muted small mx-2 text-center">Page ${pageInfo.currentPage + 1} of ${pageInfo.totalPages}</span>
                <button class="btn btn-teal px-3 px-sm-4 rounded-pill nextBtnInst" ${pageInfo.currentPage >= pageInfo.totalPages - 1 ? 'disabled' : ''} style="background-color: #20c997; color: white; border: none;">Next</button>
            </div>
        `;
    }

    const listHtml = list.map(d => `
        <div class="digi-card d-flex align-items-center p-3 p-sm-4 rounded-4 shadow-sm bg-white mb-3" 
             onclick="searchByNameApi('${d.name}')" role="button" style="cursor:pointer; border-left: 6px solid #20c997; transition: transform 0.2s;">
            <!-- Adjusted ID width for mobile -->
            <span class="text-secondary fw-bold me-2 me-sm-4 fs-6 fs-sm-5" style="min-width: 60px; sm-min-width: 80px;">#${String(d.id).padStart(4, '0')}</span>
            <h3 class="h5 h4-sm fw-bold mb-0 flex-grow-1 text-dark text-truncate">${d.name}</h3>
            <div class="img-container rounded-3 p-1 p-sm-2 bg-light shadow-sm ms-2">
                <img src="${d.image}" alt="${d.name}" style="width: 48px; height: 48px; sm-width: 64px; sm-height: 64px; object-fit: contain;">
            </div>
        </div>
    `).join('');
    ...
    return `
        <div class="digimon-detail-view w-100 animate__animated animate__fadeIn">
            <div class="text-center mb-3 mb-sm-4">
                ...
                <h2 class="h1 display-sm-5 fw-bold mb-0" style="color: #0d684d;">${name}</h2>
            </div>

            <div class="row justify-content-center g-0">
                <div class="col-lg-8 col-md-10 col-12">
                    <div class="text-center rounded-5 p-3 p-sm-4 mb-3 mb-sm-4" style="background-color: #f0fdfa;">
                        <img src="${images?.[0]?.href}" alt="${name}" class="img-fluid" style="max-height: 300px; sm-max-height: 400px; filter: drop-shadow(0 10px 15px rgba(32, 201, 151, 0.2));">
                    </div>

                    <!-- Row g-2 remains, but badges handle overflow better -->
                    ...
                    <div class="fields-container p-3 rounded-4 border bg-light mb-4 mx-1">
                        ...
                        <div class="d-flex flex-wrap gap-2 justify-content-center">
                            ${fields?.length ? fields.map(f => `
                                <div class="d-flex align-items-center bg-white border rounded-pill px-2 py-1 shadow-sm">
                                    <img src="${f.image}" alt="${f.field}" style="width: 16px; height: 16px; margin-right: 6px;">
                                    <span style="font-size: 0.75rem;" class="fw-bold">${f.field}</span>
                                </div>
                            `).join('') : '<span class="text-muted small">No fields found.</span>'}
                        </div>
                    </div>

                    <div class="skills-container bg-white border rounded-4 p-3 p-sm-4 shadow-sm mx-1">
                        ...
                        <div class="move-list">
                            ${skills?.length ? skills.slice(0, 5).map(s => `
                                <div class="mb-3 pb-2 border-bottom">
                                    ...
                                    <p class="text-muted mb-0" style="font-size: 0.85rem; line-height: 1.4;">${s.description || 'No description available.'}</p>
                                </div>
...
function renderBadgeCol(label, value, cssClass) {
    ...
    return `
        <div class="col-4">
            ...
            <div class="badge ${finalClass} w-100 py-2 text-truncate d-block" ${customStyle} style="font-size: 0.7rem;">${value || 'N/A'}</div>
        </div>`;
}